...
import { getMonthName, MONTHS, normalizeDob, isWorkerActiveInMonth, DOC_TYPES, formatDobToYYMMDD, formatDateTime, isClientActiveInMonth, getSubmissionKey, getWeeksInMonth } from '../utils/helpers';
import { SupportWorkerModal } from './common/SupportWorkerModal';

// Retroactive Detail Modal
export const RetroactiveDetailModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  items: PaymentItem[];
  clientName: string;
...
// Weekly Check Modal
export const WeeklyCheckModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    client: Client;
    year: number;
...
export const DocumentDrawerModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    client: Client;
    ...
    baseMonth: number; 
    initialTab?: number;
}> = ({ isOpen, onClose, client, setClients, submissionData, onSave, retroactiveData, allPayments, retroactiveSubmissions, setRetroactiveSubmissions, baseYear, baseMonth, initialTab }) => {
    // Initialize activeTab with initialTab if provided, otherwise baseMonth
    const [activeTab, setActiveTab] = useState(initialTab !== undefined ? initialTab : baseMonth);
